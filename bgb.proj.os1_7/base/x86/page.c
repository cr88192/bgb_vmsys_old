...
//	mem_pages=16384; /* FIXME: make not assume 64MB */
	...
	Page_Lock(256,256); /* lock 1-2M (kernel image) */
...
char *kalloc_phys(int sz)
{
	void *p;
	p=(void *)(Page_Alloc((sz+4095)>>12)<<12);
	return(p);
}
...
int Paging_Setup()
{
	...
	j=256*1024; //number of pages in kernel region

	kernel_page_table=Page_Alloc(1+(j>>10));
	ptab=(unsigned int *)(kernel_page_table<<12);

//	for(i=0;i<1024;i++)ptab[i]=0;
	for(i=0;i<(1024+j);i++)ptab[i]=0;	//clear page tables
	for(i=0;i<(j/1024);i++)ptab[i]=((kernel_page_table+i+1)<<12)|3;
...
int Paging_ForkKernel()
{
	unsigned int *ptab, *ptab2;
	int i, j, k;
	
	kprint("forking kernel page tables.\n");

	j=Paging_GetCR3();
	ptab=(unsigned int *)(j<<12);

	k=Page_Alloc(1+(mem_pages>>10));
	ptab2=(unsigned int *)(k<<12);

	for(i=0;i<1024;i++)ptab2[i]=0;
	for(i=0;i<256;i++)ptab2[i]=ptab[i];
...
int Address_FindRegionUS(int pages)
{
	unsigned int *ptab, *ltab;
	int high;
	int j, i, c, s;
...
//			kprint("Address_AllocRegion: A0\n");
			...
			{
				ltab[start+i]=(Paging_AllocRelease()<<12)|3|256;
...
int Address_AllocRegionUS(int start, int len)
{
	unsigned int *ptab, *ltab;
	int high;
	int i;
...
//	kprint("Address_AllocRegion: M0\n");
	...
	if(!start)start=Address_FindRegionUS(len);
	if(!start)return(0);
...
//	kprint("Address_AllocRegion: M1\n");
	...
	for(i=0; i<len; i++)
	{
		...
		{
//			kprint("Address_AllocRegion: N0\n");
			ptab[high]=(Paging_AllocRelease()<<12)|7;
			Paging_FlushCR3();
//			kprint("Address_AllocRegion: N1\n");
		}
		if(!(ltab[start+i]&1))
...
//			kprint("Address_AllocRegion: A0\n");
			...
			{
				ltab[start+i]=(Paging_AllocRelease()<<12)|7;
				Paging_AddPageEnd(ltab[start+i]&(~4095));
			}
//			kprint("Address_AllocRegion: A1\n");
		}
