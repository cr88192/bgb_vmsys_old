/*
Copyright (C) 2000  Brendan G Bohannon
...
based on:
fdc.c from GazOS Operating System
...
static BOOL read_block(int block,BYTE *blockbuff, unsigned long nosectors);
...
//   set_vector(floppy_ISR, M_VEC+6, (D_INT+D_PRESENT+D_DPL1));
   NewIntISR(&floppy_ISR,0x26,0);
   enable_irq(6);

//   set_vector(_int1c, 0x1c, (D_INT+D_PRESENT+D_DPL1));
	TEV_AddProc(&int1c,0.05,1);
...
static BOOL log_disk(DrvGeom *g)
{
...
static BOOL write_block(int block,BYTE *blockbuff, unsigned long nosectors)
{
...
static BOOL format_track(BYTE track,DrvGeom *g)
{
...
static int fdc_offs, fdcbufloc;
static byte fdcbuf[512];

/* FIXME: abstract to buffer cache */
static int fdc_writelin(void *buf, int len, VFILE *fd)
{
	int ilen, sz;
	...
	if(fdc_offs&511)
	{
		if((fdc_offs>>9)!=fdcbufloc)
		{
			read_block((fdc_offs>>9), fdcbuf, 1);
			fdcbufloc=(fdc_offs>>9);
		}
		sz=512-(fdc_offs&511);
		memcpy(fdcbuf+(fdc_offs&511), buf, sz);
		buf+=sz;
		len-=sz;
		fdc_offs+=sz;
		write_block(fdcbufloc, fdcbuf, 1);
	}

	sz=len>>9;
	write_block(fdc_offs>>9, buf, sz);
	len-=sz<<9;
	fdc_offs+=sz<<9;
	buf+=sz<<9;
...
static int fdc_readlin(void *buf, int len, VFILE *fd)
{
	int ilen, sz;

	ilen=len;

	KDPRINT("FDC_READLIN 1\n");
	...
	if(fdc_offs&511)
	{
		...
		sz=((len<512)?len:512)-(fdc_offs&511);
		memcpy(buf, fdcbuf+(fdc_offs&511), sz);
		buf+=sz;
		len-=sz;
		fdc_offs+=sz;
	}

	KDPRINT("FDC_READLIN 2\n");

	sz=len>>9;
	if(sz)
	{
		read_block(fdc_offs>>9, buf, sz);
		len-=sz<<9;
		fdc_offs+=sz<<9;
		buf+=sz<<9;
	}

	KDPRINT("FDC_READLIN 3\n");
	...
	if(len)
	{
		...
		memcpy(buf, fdcbuf, len);
		buf+=len;
		len-=len;
		fdc_offs+=len;
	}

	KDPRINT("FDC_READLIN 4\n");

	return(ilen);
}

static int fdc_seeklin(VFILE *fd, int adr, int rel)
{
	switch(rel)
	{
	case 0:
	...
	case 1:
		...
		break;
	default:
		Con_Printf("FDC: Invalid Seek.\n");
		break;
	}
	return(fdc_offs);
}

static int fdc_telllin(VFILE *fd)
{
	return(fdc_offs);
}

static VFILE *fdc_open(char *name, void *data)
{
	VFILE *tmp;

	if(fdc_fd)return(fdc_fd);
	tmp=vfnew();
	fdc_fd=tmp;

	tmp->write_proc=&fdc_writelin;
	tmp->read_proc=&fdc_readlin;
	tmp->seek_proc=&fdc_seeklin;
	tmp->tell_proc=&fdc_telllin;
