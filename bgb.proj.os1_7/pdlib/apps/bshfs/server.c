...
int BSHFS_HandleOpenReq(void *hdr, int hsz, void *data, int dsz, VADDR *from)
{
	...
	{
		fseek(bshfs_files[i]->fd, 0, 2);
		j=ftell(bshfs_files[i]->fd);

		open_rep->size_high=0;
		open_rep->size_low=htonl(j);
...
int BSHFS_HandleIOReq(void *hdr, int hsz, void *data, int dsz, VADDR *from)
{
	...
	{
		//HACK: a read from offset -1 fails, but the response has an
		//offset indicating the eof
		i=ntohl(io_req->offset_high);
		offs=ntohl(io_req->offset_low);
		if((i==-1) && (offs==-1))
		{
			fseek(bshfs_files[hdl]->fd, offs, 2);
			i=ftell(bshfs_files[hdl]->fd);

			io_req->type=0;
			io_req->offset_high=0;
			io_req->offset_low=htonl(i);
