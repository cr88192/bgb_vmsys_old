...
int StkComp_PushReg(elem ctx, int reg)
{
	elem x, t;


	x=VECTOR_REF(ctx, STKVM_DSP);
	t=VECTOR_REF(ctx, reg);
	x=CONS(t, x);
	VECTOR_SET(ctx, STKVM_DSP, x);
	return(0);
}

int StkComp_PopReg(elem ctx, int reg)
{
	elem x, t;

	x=VECTOR_REF(ctx, STKVM_DSP);
	t=CAR(x);
	VECTOR_SET(ctx, reg, t);
	x=CDR(x);
	VECTOR_SET(ctx, STKVM_DSP, x);
...
int StkComp_Push(elem ctx, elem val)
{
	elem x, t;
...
elem HpiComp_DefineSpecial(char *name, elem (*func)(), int pc)
{
	...
	MMGC3_BindNameAddr(name, func);

	return(MISC_NULL);
}

elem StkComp_Compile(elem form, elem next, elem ctx, int tail);
elem StkInterp_Run(elem form, elem env_n, elem env_d, elem dyn_n, elem dyn_d);

elem StkComp_CompileExpr(elem form, elem next, elem ctx, int tail)
{
	elem cur, x, t, f;
	elem ep_n, ep_d;

	t=Namespace2_Lookup(VECTOR_REF(ctx, STKVM_SEPN), VECTOR_REF(ctx, STKVM_SEPD), CAR(form));
	if(ELEM_SYNTAXP(t))
	{
		t=TyMisc_EvalSpecial(t, CONS(form, CONS(next, CONS(ctx, CONS(tail, MISC_EOL)))));
		return(t);
	}
	if(ELEM_CLOSUREP(t))
	{
...
//		ctx->ip=CONS(CONS(SYM("frame"), CONS(x, MISC_EOL)), form_halt);
//		HpiOp_Frame(ctx);

		StkComp_PushReg(ctx, STKVM_EPN);
		StkComp_PushReg(ctx, STKVM_EPD);
		StkComp_PushReg(ctx, STKVM_DYNN);
		StkComp_PushReg(ctx, STKVM_DYNN);
		StkComp_PushReg(ctx, STKVM_SEPN);
		StkComp_PushReg(ctx, STKVM_SEPD);

		ep_n=TyFcn_VectorRef(t, ELEM_FROMFIXNUM(0));
		VECTOR_SET(ctx, STKVM_EPN, ep_n);

		ep_d=TyFcn_VectorRef(t, ELEM_FROMFIXNUM(1));
		ep_d=CONS(CONS(form, MISC_EOL), ep_d);
		VECTOR_SET(ctx, STKVM_EPD, ep_d);

//		t=HpiInterp_RunCtx(ctx->ip, ctx);
		StkInt_Run(ctx);
		t=StkComp_Pop(ctx);

		x=StkComp_Compile(t, next, ctx, tail);
...
//		ctx->ip=f;
	...
	x=StkComp_Compile(CAR(form), x, ctx, 0);
	if(ELEM_UNDEFP(x))return(MISC_UNDEFINED);

	cur=CDR(form);
	while(ELEM_CONSP(cur))
	{
		x=StkComp_Compile(CAR(cur), x, ctx, 0);
		cur=CDR(cur);
	}
	if(cur!=MISC_EOL)
	{
...
//		t=HpiComp_Lookup(form, ctx->ep_n);
		t=Namespace2_Trace(ctx->ep_n, ctx->ep_d, MISC_EOL, form);
...
elem HpiSpecial_BoundP(elem form, elem next, InterpCtx *ctx, elem tail)
{
	...
	x=Namespace2_Trace(ctx->ep_n, ctx->ep_d, MISC_EOL, CADR(form));
	if(x==MISC_UNDEFINED)
	{
...
elem HpiSpecial_DefineSyntax(elem form, elem next, InterpCtx *ctx, elem tail)
{
	...
	t=ctx->ip;
	x=HpiComp_Compile(CADDR(form), form_halt, ctx, 0);
...
//		x=CONS(CONS(SYM("assign"), CONS(t, MISC_EOL)), next);
		...
		x=HpiComp_Compile(CADDDR(form), form_halt, ctx, 0);
		HpiComp_PopEnv(ctx);
...
//	x=CONS(CONS(SYM("return"), MISC_EOL), MISC_EOL);
	...
	ctx->ep_n=lbinds_n;
	ctx->ep_d=lbinds_d;
//	x=HpiComp_Compile(CADDR(form), form_return, ctx, 1);
	x=HpiSpecial_Begin(CDR(form), form_return, ctx, 1);
	HpiComp_PopEnv(ctx);
...
//	t=HpiComp_Compile(CADDR(form), x, ctx, 1);
	t=HpiSpecial_Begin(CDR(form), form_return, ctx, 1);
	HpiComp_PopEnv(ctx);
	...
	while(cur!=MISC_EOL)
	{
		if(ELEM_CONSP(CAR(cur)))
		{
			if(CAR(CAR(cur))==SYM("unquote"))
				lst=CONS(CONS(SYM("#uqf"), HpiComp_Compile(CADR(CAR(cur)), form_halt, ctx, 0)), lst);
				else if(ELEM_CONSP(CAR(cur)))if(CAR(CAR(cur))==SYM("unquote-splicing"))
				lst=CONS(CONS(SYM("#uqs"), HpiComp_Compile(CADR(CAR(cur)), form_halt, ctx, 0)), lst);
	...
	while(ELEM_CONSP(cur))
	{
		if(ELEM_CONSP(CAR(cur)))
		{
			...
			}else
			{
				...
				t=CONS(CONS(SYM("xchg-rp-ax"), MISC_EOL), form_return);
...
//	x=HpiComp_Compile(CADR(CAR(expr)), form_return, ctx, 1);
	t=CONS(SYM("begin"), CDR(CAR(expr)));
	x=HpiComp_Compile(t, form_return, ctx, 1);
...
int HpiComp_Init()
{
//	MMGC3_AddRoot(&hpicomp_specials_ntree);
//	MMGC3_AddRoot(&hpicomp_specials_dtree);

	MMGC3_AddRoot(&form_apply);
	MMGC3_AddRoot(&form_return);
	MMGC3_AddRoot(&form_halt);
	MMGC3_AddRoot(&form_frame_apply_halt);
...
//	HpiComp_DefineSpecial("doubleqq", &HpiSpecial_DoubleQQ, 4);
	...
	HpiComp_DefineSpecial(":=", &HpiSpecial_SetN, 4);
