...
{
	ctx->sp=CONS(ctx->dyn_d, CONS(ctx->dyn_n, ctx->sp));
}

elem HpiComp_PopDynEnv(InterpCtx *ctx)
{
	ctx->dyn_d=CAR(ctx->sp);
	ctx->sp=CDR(ctx->sp);
	ctx->dyn_n=CAR(ctx->sp);
	ctx->sp=CDR(ctx->sp);
}
...
//	if(ELEM_SYMBOLP(CADR(form)))return(HpiSpecial_NamedLet(form, next, ctx, tail));
	...
	r=TyFcn_NReverse(r);
	lbinds_n=CONS(r, ctx->dyn_n);
	lbinds_d=CONS(r, ctx->dyn_d);

	x=CONS(TyFcn_Symbol("return"), MISC_EOL);
	x=CONS(x, MISC_EOL);

	HpiComp_PushDynEnv(ctx);
	ctx->dyn_n=lbinds_n;
	ctx->dyn_d=lbinds_d;
	t=HpiComp_Compile(CADDR(form), x, ctx, 1);
	HpiComp_PopDynEnv(ctx);
