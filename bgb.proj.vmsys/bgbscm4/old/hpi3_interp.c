...
elem HpiInterp_RunStep(InterpCtx *ctx)
{
	...
	}else switch(ELEM_TYPE3(CAR(ctx->ip)))
	{
	case TYPE_FIXNUM:
		ctx->rp=CONS(CAR(ctx->ip), ctx->rp);
		ctx->ip=CDR(ctx->ip);
		break;
	case TYPE_OBJECT:
		switch(OBJECT_TYPE(CAR(ctx->ip)))
		{
		case OBJECT_SYMBOL:
			op=Namespace2_Lookup(ctx->dyn_n, ctx->dyn_d, CAR(ctx->ip));
			if(op==MISC_UNDEFINED)op=Namespace2_Lookup(ctx->ep_n, ctx->ep_d, CAR(ctx->ip));
			if(op==MISC_UNDEFINED)
			{
				sa_print("HpiInterp_RunStep: unbound var \"%s\"\n", ELEM_TOSYMBOL(CAR(ctx->ip)));
				break;
			}
			HpiInterp_MiniApply(ctx, op);
			break;
...
int HpiInterp_Init()
{
	MMGC3_AddRoot(&opcodes_n);
	MMGC3_AddRoot(&opcodes_d);
...
//	HpiInterp_DefineSpecial("current-envobj",	&HpiOp_CurrentEnvObj, 1);
	...
	HpiInterp_DefineSpecial("func",				&HpiOp_Func, 1);
