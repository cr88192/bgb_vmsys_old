...
elem tyfcn_symvec=MISC_NULL;
elem tyfcn_strlist=MISC_EOL;
static int tyfcn_init=0;
...
int TyFcn_Init()
{
	...
	if(tyfcn_init)return(0);
	tyfcn_init=1;

	sa_print("TyFcn_Init\n");

//	SAGC_AddRoot(&tyfcn_symlist);
	SAGC_AddRoot(&tyfcn_symvec);
	SAGC_AddRoot(&tyfcn_strlist);
	SAGC_AddRoot(&tyfcn_free_cons);

	if(tyfcn_symvec==MISC_NULL)
	{
		tyfcn_symvec=VECTOR_NEW(1024);
		for(i=0; i<1024; i++)VECTOR_SET(tyfcn_symvec, i, MISC_EOL);
	}
...
elem TyFcn_SymCount()
{
//	return(TyFcn_ListLength(tyfcn_symlist));
}
...
//		tyfcn_free_cons=CDR(tyfcn_free_cons);
	...
	{
		tyfcn_free_cons=sa_multicons(16384, tyfcn_free_cons);
		return(TyFcn_Cons(a, b));
	}

//	t=sa_alloc(2*ELEM_SZ);
//	t=kpgcons();

//	t=ELEM_TOCONS(SAGC_Cons());
	t=sa_cons();
...
elem TyFcn_CarEv(elem c, elem ctx)
{
	...
	if(ELEM_TYPEXP(t))
	{
		t2=TypeX_EvalMulti(t, MISC_NULL, ctx);
		if(t2!=MISC_UNDEFINED)return(t2);
		t2=TypeX_EvalSingle(t, ctx);
...
elem TyFcn_CopyVector(elem a)
{
	...
	if(!ELEM_VECTORP(a))
	{
		sa_fatal("TyFcn_CopyVector: non-vector.\n");
		return(MISC_UNDEFINED);
	}
...
elem TyFcn_WidenumIs(elem a, elem ty)
{
	...
	if(!ELEM_WIDENUMP(a))return(MISC_FALSE);
...
elem *TyFcn_VectorBody(elem a)
{
	...
	if(!ELEM_VECTORP(a))
	{
		sa_error("TyFcn_VectorBody: non-vector.\n");
		return(NULL);
	}
	...
	if(ty!=VECTOR_ELEMENT)
	{
		sa_error("TyFcn_VectorBody: vector non-elem.\n");
		return(NULL);
	}
...
unsigned char *TyFcn_ByteVectorBody(elem a)
{
	elem *s;
	int l, ty;
	...
	if((ty!=VECTOR_U8) && (ty!=VECTOR_S8))
	{
		sa_error("TyFcn_ByteVectorBody: vector non-byte.\n");
		return(NULL);
	}
...
elem TyFcn_StringEP(elem a, elem b)
{
	...
	l=(s[0]>>16)&4095;
//	if(((s[0]>>16)&65535)!=((t[0]>>16)&65535))return(MISC_FALSE);
//	l=(s[0]>>16)&65535;
...
//	cur=tyfcn_symlist;
	cur=*lst;
	while(cur!=MISC_EOL && c)
	{
		if(!ELEM_SYMBOLP(CAR(cur)))
		{
			sa_print("TyFcn_Symbol: type check %X\n",
				CAR(cur));
			cur=CDR(cur);
			continue;
		}
	...
	if(!c)sa_fatal("TyFcn_Symbol: runaway loop\n");
...
elem TyFcn_Symbol(char *s)
{
	...
	x=VECTOR_REF(tyfcn_symvec, j);
	t2=TyFcn_SymbolList(s, &x);
	VECTOR_SET(tyfcn_symvec, j, x);
...
elem TyFcn_HashString(elem s)
{
	...
	while(cur!=MISC_EOL && c)
	{
		...
		if(t==MISC_TRUE)
		{
			...
			{
				TyFcn_SetCdr(w, CDR(cur));
				TyFcn_SetCdr(cur, tyfcn_strlist);
				tyfcn_strlist=cur;
			}
			break;
		}
	...
	if(!c)sa_fatal("TyFcn_HashString: runaway loop\n");

	if(u==MISC_NULL)
	{
		u=s;
		tyfcn_strlist=CONS(u, tyfcn_strlist);
	}
	return(u);
}
...
elem TyFcn_ReverseTree(elem lst)
{
	...
	if(!ELEM_CONSP(lst))return(lst);

	cur=lst;
	t=MISC_EOL;
	while(ELEM_CONSP(cur))
	{
		t=CONS(TyFcn_ReverseTree(CAR(cur)), t);
		cur=CDR(cur);
	}
	return(t);
}
...
elem TyFcn_MatchP(elem lit, elem a, elem b)
{
	...
	if(ELEM_SYMBOLP(a))
	{
		...
		if(t!=MISC_FALSE || ELEM_SYMKEYWORDP(a))
			if(TyFcn_KeywordBase(a) !=
				TyFcn_KeywordBase(b))
				return(MISC_FALSE);
		return(MISC_TRUE);
	}
...
elem TyFcn_MatchV(elem a, elem b)
{
	elem cur, cur2, t;
	int i, j, c;
	static elem sym_UK=MISC_NULL;
	static elem sym_P3=MISC_NULL;
...
#if 0
	if(ELEM_CLOSUREP(a) || ELEM_FUNCTIONP(a))
	{
		t=Interp_ApplyCtxSafe(interp_repl_ctx, a, CONS(b, MISC_EOL));
		if(t==MISC_FALSE)return(t);
	...
	if(ELEM_SYMBOLP(a))
	{
		if(ELEM_SYMKEYWORDP(a))
		{
			if(a==SYMG(sym_UK, "_:") &&
				ELEM_SYMKEYWORDP(b))
				return(ELEM_FROMFIXNUM(0));
				/* vague match keyword */

			if(TyFcn_KeywordBase(a)==TyFcn_KeywordBase(b))
				return(ELEM_FROMFIXNUM(1));
	...
	if(ELEM_CONSP(a) && ELEM_CONSP(b))
	{
		...
		while(ELEM_CONSP(cur) && ELEM_CONSP(cur2))
		{
			if(CAR(cur)==SYMG(sym_P3, "..."))return(MISC_TRUE);
...
//	printf("objbuf %p\n", buf);
	...
	case OBJECT_VECTOR:
		if(((buf[0]>>8)&255)>11)
		{
			sa_print("#<vector-type %d %X>", ((buf[0]>>8)&255), obj);
			break;
		}
...
//			TyFcn_DumpElem(buf[i+2]);
	...
	case OBJECT_TYPEX:
		...
		break;
	default:
		sa_print("#<obj-type %d %X>", buf[0]&255, obj);
		break;
	}
	return(0);
}
...
int TyFcn_DisplayElem(elem obj)
{
	TyPrint_PrintFlags(obj, FIXNUM(TYPRINT_DISPLAY));
	return(0);
}

int TyFcn_DumpElem(elem obj)
{
	TyPrint_PrintDefaults(obj);
	return(0);
}
