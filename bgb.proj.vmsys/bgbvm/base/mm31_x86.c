...
//	tmp=KPFH_BasicMMap(NULL, size, KPROT_READ|KPROT_WRITE, KMAP_ANONYMOUS, NULL, 0);
...
/*
		vvfseek(mmgc3_storeimg, (schk<<18)+(spage<<12), 0);
		vvfread((void *)page, 1, 4096, mmgc3_storeimg);
*/
...
mem3chunk *MMGC3_LoadChunk(int num)
{
	...
	kprint("MMGC3_LoadChunk, check %d %X.\n", num);
...
#if 1
//	cur=kalloc(MINCHKSZ);
	cur=(mem3chunk *)(Address_AllocRegion(0, MINCHKSZ>>PAGE_SHIFT)<<12);
