...
//	kprint("MM2_NewBlock: M1\n");

//	head=malloc(size2);
//	head=(mem2block *)(Page_Alloc(size2>>PAGE_SHIFT)<<PAGE_SHIFT);
	head=(mem2block *)(Address_AllocRegion(0, size2>>PAGE_SHIFT)<<12);
...
{
//	kprint("MM2 Init2...\n");
	swap_fd=Driver_OpenStream("ata0.2", NULL);
	swap_region=vfmmap(NULL, 128*1024*1024, 0, MAP_FIXED|MAP_FORCE, swap_fd, 0);
	swap_end=swap_region;

//	free_list=NULL; /* trash free list to force allocation of more data */
}

mem2block *MM2_NewBlock2(int size)
{
	int i, j, k;
	int s, size2;
	mem2block *head;

	kprint("MM2_NewBlock2.\n");
