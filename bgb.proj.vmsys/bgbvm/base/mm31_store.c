...
int MMGC3_UnmapChunk(mem3chunk *chk)
{
	int i;
	...
	kprint("unmap chunk %d\n", i);
	MMGC3_MapSetEnt(mmgc3_storemap, i, 0);
...
elem MMGC3_SwizRefOut(elem obj)
{
	...
	if(!cur)
	{
		sa_error("MMGC3_SwizRefOut: no chunk (%X).\n", p, &obj);
...
//	sa_print("MMGC3_SwizRefIn: %X -> %d %d\n", obj, obj>>18, i>>3);
	if(((MMGC3_ChunkGetEnt(cur, i>>3)&3)!=1) &&
		((MMGC3_ChunkGetEnt(cur, i>>3)&3)!=2))
		sa_print("\tswiz in: invalid cell %d (%d %d)\n", MMGC3_ChunkGetEnt(cur, i>>3), obj>>18, i);
...
//					if(j==34)kprint("vec type %d, len %d, addr %p.\n", j, h, t);
			...
			case OBJECT_TYPEX:
				TypeX_SwizOut(obj);
				t[1]=MMGC3_SwizRefOut(TypeX_Sym(obj));
				break;
			default:
...
//			cells=1<<(((chk->size_rover)>>27)-3);
			...
			case OBJECT_SYNTAX:
				...
				break;
			case OBJECT_TYPEX:
				t[1]=(elem)TypeX_GetInfo(ELEM_TOSYMBOL(
					MMGC3_SwizRefIn(t[1])));
				TypeX_SwizIn(obj);
				break;
			default:
...
int MMGC3_SetStoreImage(char *name)
{
	mmgc3_storefile=kstrdup(name);
	return(0);
}
...
int MMGC3_RestoreImage()
{
	...
	mmgc3_storeimg=vffopen(mmgc3_storefile, "r+b");
	if(!mmgc3_storeimg)mmgc3_storeimg=vffopen(mmgc3_storefile, "w+b");
	if(!mmgc3_storeimg)
	{
		kprint("failed open store image '%s'.\n", mmgc3_storefile);
		return(-1);
	}
//	return(0);
