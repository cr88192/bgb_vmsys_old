...
elem VPathVM_WrapVFile(VFILE *fd)
{
	elem t;
...
VFILE *VPathVM_UnwrapVFile(elem f)
{
	VFILE *fd;

//	fd=*(VFILE **)TyFcn_TypeX_Data(f);
	fd=MMGC3_GetCData(f);
...
//		fd=MMGC3_GetCData(t);
		fd=VPathVM_UnwrapVFile(t);
	}else
	{
//		fd=MMGC3_GetCData(CAR(rest));
		fd=VPathVM_UnwrapVFile(CAR(rest));
	}
...
elem VPathVM_Apply(elem port, elem rest)
{
}
