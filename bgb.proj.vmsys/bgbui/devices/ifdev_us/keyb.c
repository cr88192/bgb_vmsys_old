...
int keyb_handler(void *wnd, int key, int s)
{
//	byte *keymap;
	int akey;
...
//	keymap=Window_GetKeyMap(gfx_screen);
	...
	if(s && (key<128))
	{
		con_keymap[key/8]|=1<<(key&7);
		akey=keyb_map_us[akey];
//		key=keyb_map_ru_yasherty[akey];

		if(con_keybuf_pos<60)
		{
			con_keybuf[con_keybuf_pos++]=akey>>8;
			con_keybuf[con_keybuf_pos++]=akey&0xff;
		}
	}

	if(s)
		con_keymap[key/8]|=1<<(key&7);
		else con_keymap[key/8]&=~(1<<(key&7));
