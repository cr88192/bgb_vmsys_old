...
int main(int argc, char *argv[])
{
	...
	for(i=0; i<pheader->numframes; i++)
	{
		...
		for(j=0; j<pheader->numverts; j++)
			...
			(poseverts[i][j].v[0]*pheader->scale[0])+pheader->scale_origin[0],
			(poseverts[i][j].v[1]*pheader->scale[1])+pheader->scale_origin[1],
			(poseverts[i][j].v[2]*pheader->scale[2])+pheader->scale_origin[2],
			stverts[j].s, stverts[j].t, stverts[j].onseam);
		fprintf(outfd, "END\n");
	}
	fprintf(outfd, "END\n");
